...
export async function POST(request) {
  try {
    ...
    if (action === 'start_flow') {
      const { content, from = 'Boss', agent = 'main', messageId, delegatedTo } = body
      ...
      if (messageId) {
        ...
        if (req) {
          ...
          if (content) {
            ...
            createEvent(req.id, 'received', 'main', `📥 Request from ${from}: "${cleanText.slice(0, 60)}${cleanText.length > 60 ? '...' : ''}"`)
          }
        }
      }
      ...
      if (!req) {
        ...
        if (placeholder && (placeholder.state === 'received' || placeholder.state === 'analyzing')) {
          ...
          if (content) {
            ...
            createEvent(placeholder.id, 'received', 'main', `📥 Request from Boss: "${cleanText.slice(0, 60)}${cleanText.length > 60 ? '...' : ''}"`)
          }
        }
      }
      ...
      if (delegatedTo && delegatedTo !== 'main') {
        ...
        if (isChainContinuation && previousAgent) {
          ...
          eventBus.emit(EVENTS.WORKFLOW_EVENT, {
            ...
            message: `📨 ${AGENTS[previousAgent]?.name || previousAgent} returning results to ${'司礼监'}`,
            targetAgent: 'main',
            time: timeStr(),
            timestamp: Date.now(),
            chainId,
          ...
          setTimeout(() => {
            ...
            createEvent(req.id, 'reviewing', 'main', `🔄 Reviewing results from ${AGENTS[previousAgent]?.name || previousAgent}...`)
            emitRequestUpdate(req.id)
          }, 1500)
        }
        ...
        setTimeout(() => {
          ...
          createEvent(req.id, 'analyzing', 'main', `🔍 Analyzing: "${cleanText.slice(0, 50)}${cleanText.length > 50 ? '...' : ''}"`)
          emitRequestUpdate(req.id)
        }, chainDelay + 500)
        ...
        setTimeout(() => {
          ...
          createEvent(req.id, 'task_created', 'main', `📋 Task → ${AGENTS[delegatedTo]?.emoji || '🤖'} ${AGENTS[delegatedTo]?.name || delegatedTo}: "${task.title}"`)
          emitRequestUpdate(req.id)
        }, chainDelay + 1200)
        ...
        if (req) {
          setTimeout(() => {
            eventBus.emit(EVENTS.WORKFLOW_EVENT, {
              ...
              agentColor: AGENTS[agent]?.color || '#888',
              agentName: AGENTS[agent]?.name || agent,
              message: `📨 ${agentName} returning results to ${'司礼监'}`,
              targetAgent: 'main',
              time: timeStr(),
              timestamp: Date.now(),
            })
          ...
          setTimeout(() => {
            createEvent(task.requestId, 'delivering', 'main', `📬 ${'司礼监'} received results from ${agentName}`)
            emitRequestUpdate(task.requestId)
          }, 2500)
        }
      }
    ...
    if (action === 'delegate_complete') {
      ...
      const effectiveAgent = agent || task.assignedAgent || 'main'
    ...
    if (action === 'quick_flow') {
      ...
      if (!req) {
        ...
        incrementMessages('received')
        createEvent(req.id, 'received', 'main', `📥 Request from ${from}: "${content.slice(0, 60)}${content.length > 60 ? '...' : ''}"`)
        emitRequestUpdate(req.id)
      }
      ...
      if (!alreadyAnalyzing) {
        setTimeout(() => {
          ...
          createEvent(requestId, 'analyzing', 'main', `🔍 Analyzing: "${content.slice(0, 50)}${content.length > 50 ? '...' : ''}"`)
        }, baseDelay)
      }
      ...
      setTimeout(() => {
        ...
        updateRequest(requestId, { state: 'task_created', task: { id: task.id, title: task.title, detail: task.detail, targetAgent: agent, reason: reason || `Assigned by ${'司礼监'}` } })
        emitRequestUpdate(requestId)
        emitTaskUpdate(task.id)
        createEvent(requestId, 'task_created', 'main', `📋 Task created → ${AGENTS[agent]?.name || agent}: ${reason || `Assigned by ${'司礼监'}`}`)
      }, baseDelay + 1500)
      ...
      setTimeout(() => {
        ...
        emitRequestUpdate(requestId)
        const isSelf = agent === 'main'
        const r = getRequestById(requestId)
        createEvent(requestId, 'assigned', 'main', 
    ...
    if (action === 'complete') {
      ...
      const savings = recordTaskCompletion(req.assignedTo || 'main', taskTimeMs)
      incrementMessages('sent')
      if (tokensInput > 0 || tokensOutput > 0) addTokens(tokensInput, tokensOutput)
    ...
    if (action === 'manual_complete') {
      ...
      createEvent(req.id, 'completed', req.assignedTo || 'main', `✅ Completed: "${req.task?.title}" - ${result || 'Done'}`)
    ...
    if (action === 'clear_pipeline') {
      ...
      if (clearedRequests > 0 || clearedTasks > 0) {
        createEvent(null, 'system', 'main', `🔄 Pipeline cleared: ${clearedRequests} request${clearedRequests !== 1 ? 's' : ''}, ${clearedTasks} task${clearedTasks !== 1 ? 's' : ''} completed (${reason})`)
    ...
    if (action === 'analyze') {
      ...
      createEvent(req.id, 'analyzing', 'main', `🔍 Analyzing: "${req.content.slice(0, 40)}..."`)
    ...
    if (action === 'create_task') {
      ...
      emitRequestUpdate(requestId)
      emitTaskUpdate(task.id)
      createEvent(req.id, 'task_created', 'main', `📋 Task created → ${AGENTS[analysis.agent]?.name || analysis.agent}: ${analysis.reason}`)
    ...
    if (action === 'assign') {
      ...
      emitRequestUpdate(requestId)
      const isSelf = req.task.targetAgent === 'main'
      createEvent(req.id, 'assigned', 'main', isSelf ? `📧 Taking this one myself: "${req.task.title}"` : `📧 Delegating to ${AGENTS[req.task.targetAgent]?.name}: "${req.task.title}"`, { targetAgent: req.task.targetAgent })
      return Response.json({ success: true, request: getRequestById(requestId), assignedTo: req.task.targetAgent, isSelfAssigned: isSelf, nextState: 'in_progress', stateConfig: STATE_CONFIG.assigned, animation: { from: 'main', to: req.task.targetAgent, taskTitle: req.task.title } })
    }
