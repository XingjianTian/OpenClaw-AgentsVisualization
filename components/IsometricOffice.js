...
export default function IsometricOffice({ activeRequest }) {
  ...
  useEffect(() => {
    ...
    for (const evt of latestActivities) {
      if (evt.state === 'chain_return' && !processedChainReturns.current.has(evt.id)) {
        ...
        const mainAgentId = agents.find(a => a.id === 'main')?.id || agents[0]?.id || 'main'
        const fromAgent = evt.agent
        const toPos = animPositions[mainAgentId]
        const fromPos = animPositions[fromAgent] || animPositions[mainAgentId]
  ...
  }, [mounted, streamedActivities, agents, animPositions])
  ...
  useEffect(() => {
    ...
    activeRequests.forEach(req => {
      ...
      if (req.state === 'assigned' && prevState && prevState !== 'assigned') {
        const targetAgent = req.assignedTo || req.task?.targetAgent || mainAgentId
        if (targetAgent !== mainAgentId) {
          // Trigger flying task animation from WickedMan to target agent
          const from = animPositions[mainAgentId]
          const to = animPositions[targetAgent] || animPositions[mainAgentId]
      ...
      if (req.state === 'reviewing') {
        ...
        // Orchestrator is routing — show on orchestrator only
        newWorking[mainAgentId] = {
          task: { detail: req.content, title: req.content?.slice(0, 50), state: req.state },
          startedAt: req.createdAt || Date.now(),
          requestId: req.id,
        ...
        // Mail is flying — show on orchestrator (sending), target only gets the flying animation
        newWorking[mainAgentId] = {
          task: { detail: req.content, title: req.content?.slice(0, 50), state: req.state },
          startedAt: req.createdAt || Date.now(),
          requestId: req.id,
  ...
  }, [mounted, streamedRequests, agents, animPositions])
  ...
  const triggerTaskAnimation = useCallback((fromAgent, toAgent, taskTitle) => {
    ...
    const to = animPositions[toAgent] || animPositions[mainAgentId]
