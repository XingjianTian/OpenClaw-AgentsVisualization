...
db.exec(`
  ...
  INSERT OR IGNORE INTO connection_state (id) VALUES (1);
`)
...
try {
  ...
  try {
    ...
    if (err.message?.includes('duplicate column')) {
      console.log('[DB] task_id column already exists, skipping')
    } else {
      console.error('[DB] Error adding task_id:', err.message)
    }
  }
}
...
try {
  ...
  try {
    ...
    if (err.message?.includes('duplicate column')) {
      console.log('[DB] source column already exists, skipping')
    } else {
      console.error('[DB] Error adding source:', err.message)
    }
  }
}
...
try {
  ...
  try {
    ...
    if (err.message?.includes('duplicate column')) {
      console.log('[DB] tg_message_id column already exists, skipping')
    } else {
      console.error('[DB] Error adding tg_message_id:', err.message)
    }
  }
}
...
try {
  ...
  try {
    ...
    if (err.message?.includes('duplicate column')) {
      console.log('[DB] chain_id column already exists, skipping')
    } else {
      console.error('[DB] Error adding chain_id:', err.message)
    }
  }
}
...
// Connection state management (for sharing WebSocket status across module instances)
export function getConnectionState() {
  try {
    const row = db.prepare('SELECT * FROM connection_state WHERE id = 1').get()
    return {
    ...
    console.error('[DB] Failed to get connection state:', err.message)
    return { connected: false, url: '', currentRequestId: null, currentRunId: null }
  }
}

export function setConnectionState(connected, url, currentRequestId = null, currentRunId = null) {
  try {
    db.prepare(`
      UPDATE connection_state 
      SET connected = ?, url = ?, current_request_id = ?, current_run_id = ?, updated_at = ?
      WHERE id = 1
    `).run(
      connected ? 1 : 0,
      url || '',
    ...
    console.error('[DB] Failed to set connection state:', err.message)
  }
}
