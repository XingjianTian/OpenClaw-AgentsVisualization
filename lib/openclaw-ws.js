...
let animationTimers = []
// FIX: Use Map to store request for each runId (supports concurrent agents)
const runRequestMap = new Map()
// FIX: Use Map to store agent for each runId (supports concurrent agents)
const runAgentMap = new Map()
...
/**
 * Check if the current request's task is delegated (assigned to non-main agent).
...
function isTaskDelegated(requestId) {
  ...
  if (!task) {
    ...
    const req = getRequestById(requestId)
    return req && req.assignedTo && req.assignedTo !== 'main'
  }
  return task.assignedAgent && task.assignedAgent !== 'main'
}
...
function runCinematicAnimation(reqId, agentId = 'main') {
  ...
  if (currentIdx < 1) {
    ...
    const t1 = setTimeout(() => {
      ...
      createDashboardEvent(reqId, 'analyzing', agentId, `🔍 Analyzing: "${freshContent.slice(0, 50)}${freshContent.length > 50 ? '...' : ''}"`)
  ...
  const tFallback = setTimeout(() => {
    ...
    const agent = r.assignedTo || 'main'
    updateRequest(reqId, { state: 'task_created', task: { title: (r.content || '').slice(0, 50), detail: r.content, targetAgent: agent } })
    createDashboardEvent(reqId, 'task_created', agentId, `📋 Task created: "${(r.content || '').slice(0, 40)}"`)
    emitRequestUpdate(reqId)
    setTimeout(() => {
      updateRequest(reqId, { state: 'assigned', assignedTo: agent })
      createDashboardEvent(reqId, 'assigned', agentId, `📧 Assigned to ${AGENTS[agent]?.emoji || '🤖'} ${AGENTS[agent]?.name || agent}`)
      emitRequestUpdate(reqId)
      setTimeout(() => {
...
function runDelegationAnimation(reqId, delegatedTo, taskDetail, orchestratorAgent = 'main') {
  ...
  createDashboardEvent(reqId, 'task_created', orchestratorAgent, `📋 Task: "${taskDetail.slice(0, 40)}${taskDetail.length > 40 ? '...' : ''}" → ${agentName}`)
  emitRequestUpdate(reqId)
...
/**
 * Complete main's active task (only if NOT delegated)
 */
function completeAgentTask(reqId, agent = 'main') {
  if (!reqId) return
  ...
  createDashboardEvent(reqId, 'completed', agent, `✅ Done: "${cleanTitle.slice(0, 60)}${cleanTitle.length > 60 ? '...' : ''}"`)
  emitRequestUpdate(reqId)
...
function handleAgentEvent(payload) {
  ...
  const currentAgentId = agentMatch ? agentMatch[1] : 'main'
  
  // FIX: Store agent by runId for concurrent support
  if (runId) {
    runAgentMap.set(runId, currentAgentId)
  }
  
  console.log(`[openclaw-ws] 📦 RAW EVENT: agent=${currentAgentId} runId=${runId?.slice(0, 8)}`)
  ...
  if (stream === 'lifecycle') {
    ...
    if (phase === 'start') {
      // Each agent gets its own request using runId
      ...
      if (!req) {
        createRequest({
          ...
          agentName: AGENTS[currentAgentId]?.name || currentAgentId,
          agentColor: AGENTS[currentAgentId]?.color || '#888',
        ...
        console.log(`[openclaw-ws] Created request for ${currentAgentId}: ${reqId}`)
      }
      
      // Store for this runId
      if (runId) {
        runRequestMap.set(runId, reqId)
        runAgentMap.set(runId, currentAgentId)
      }
      
      // Create analyzing event
      updateRequest(reqId, { state: 'analyzing' })
      createDashboardEvent(reqId, 'analyzing', currentAgentId, `🔍 Analyzing...`)
      emitRequestUpdate(reqId)
    }
    
    if (phase === 'end') {
      console.log(`[openclaw-ws] phase:end for runId=${runId?.slice(0,8)}`)
      clearAnimationTimers()
      
      // Get request for this specific runId
      const reqId = runId ? runRequestMap.get(runId) : null
      const agentToComplete = runId ? runAgentMap.get(runId) || 'main' : 'main'
      
      if (reqId) {
        console.log(`[openclaw-ws] Completing request: ${reqId} for agent: ${agentToComplete}`)
        completeAgentTask(reqId, agentToComplete)
        
        // Clean up maps
        runRequestMap.delete(runId)
        runAgentMap.delete(runId)
      } else {
        console.log(`[openclaw-ws] No request found for runId=${runId?.slice(0,8)}`)
      }
    }
  }
    ...
    if (state === 'done' || state === 'error' || state === 'aborted') {
      ...
      if (currentRequestId) {
        ...
        if (req && req.state !== 'completed') {
          const agentToComplete = runId ? runAgentMap.get(runId) || 'main' : 'main'
          completeAgentTask(currentRequestId, agentToComplete)
        }
        currentRequestId = null
        currentRunId = null
        hasStartedStreaming = false
  ...
  if (stream === 'tool') {
    ...
    if (phase === 'start') {
      ...
      if (name === 'sessions_spawn') {
        ...
        runDelegationAnimation(reqId, delegatedTo, taskDetail, currentAgentId)
        return
      }
      
      // First non-spawn tool: advance to in_progress with main
      if (!firstToolSeen) {
        firstToolSeen = true
        clearAnimationTimers()
        const agent = req?.assignedTo || 'main'
        ...
        clearAnimationTimers()
        updateRequest(reqId, { state: 'in_progress', workStartedAt: Date.now() })
        createDashboardEvent(reqId, 'in_progress', currentAgentId, `⚡ Working...`)
        emitRequestUpdate(reqId)
      }
      
      const toolLabel = getToolLabel(name, args)
      createDashboardEvent(reqId, 'in_progress', currentAgentId, toolLabel)
    }
  }
    ...
    if (text && text.length > 0 && !text.startsWith('Read HEARTBEAT') && !text.includes('HEARTBEAT_OK') && !text.startsWith('/')) {
      ...
      if (!currentRequestId) {
        ...
        if (pending) {
          ...
          incrementMessages('received')
          createDashboardEvent(newReqId, 'received', currentAgentId, `📥 Request from Boss: "${content.slice(0, 50)}${content.length > 50 ? '...' : ''}"`)
  ...
  if (stream === 'assistant') {
    if (!hasStartedStreaming) {
      ...
      if (req && req.state !== 'in_progress' && req.state !== 'completed') {
        ...
        createDashboardEvent(reqId, 'in_progress', freshReq?.assignedTo || 'main', `✍️ Responding: "${cleanTitle.slice(0, 60)}${cleanTitle.length > 60 ? '...' : ''}"`)
        emitRequestUpdate(reqId)
      }
    }
  }
      ...
      try {
        ...
        if (msg.type === 'event' && msg.event === 'chat') {
          ...
          if (state === 'delivered' || state === 'idle') {
            clearAnimationTimers()
            if (currentRequestId) {
              completeAgentTask(currentRequestId)
              currentRequestId = null
              currentRunId = null
              hasStartedStreaming = false
            }
