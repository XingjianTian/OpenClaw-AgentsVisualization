 ...
 */
function completeWickedmanTask(reqId, agent = 'main') {
  if (!reqId) return
  ...
  if (stream === 'lifecycle') {
    ...
    if (phase === 'start') {
      ...
      if (!reqId) {
        ...
        // FIX: Store request by runId for concurrent support
        if (runId) {
          runRequestMap.set(runId, newReqId)
        }
      } else {
        runCinematicAnimation(reqId, currentAgentId)
      }
    }
    ...
    if (phase === 'end') {
      ...
      // FIX: Find request by runId first (supports concurrent agents)
      let reqIdToComplete = runId ? runRequestMap.get(runId) : currentRequestId
      if (!reqIdToComplete) {
        reqIdToComplete = currentRequestId
      }
      if (!reqIdToComplete) {
      ...
      if (currentRequestId) {
        if (isTaskDelegated(currentRequestId)) {
          ...
          if (req && req.state !== 'completed') {
            // FIX: Get agent from runAgentMap
            const agentToComplete = runId ? runAgentMap.get(runId) || 'main' : 'main'
            completeWickedmanTask(currentRequestId, agentToComplete)
          }
          if (req && req.state !== 'completed') {
